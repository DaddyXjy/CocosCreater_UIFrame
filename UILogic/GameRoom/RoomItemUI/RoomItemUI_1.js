...
cc.Class({
    ...
    refreshUI() {
        ...
        this.setCountDown(roomData.time);
        this.setDownName(roomData.gameState);
        this.setXianHong(common.util.ConvertMoneyToClient(roomData.downLine),common.util.ConvertMoneyToClient(roomData.upLine));
        this.setIsBanker(roomData.banker);
    },
    ...
    onUpdateReward(_data) {
        ...
        if(this.roomID == _data.roomId){
            if(_data.state == 2){
                this._awardTotal.push(_data.opens)
                this.refreshAwardCount();
                this.setAllAward();
            }
            this.setCountDown(_data.time);
            this.setDownName(_data.state);
        }
    },

    refreshAwardCount(){
        let awardCount = [0,0,0,0,0,0,0,0];
        this._awardTotal.forEach(index => {
            awardCount[index]++;
        });
        for (let index = 0; index < awardCount.length; index++) {
            const ele = awardCount[index];
            var a  = this["_img_animal_"+index].getChildByName('_label_count1');
            var b = a.$Label;
            b.string = ele;
        }
    },
